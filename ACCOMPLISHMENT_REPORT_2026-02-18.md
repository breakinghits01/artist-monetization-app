...
## � Startup After Machine Restart

### Quick Start (PM2 Auto-Start)
If PM2 is configured to auto-start on boot:
```bash
# PM2 should automatically start all services
pm2 status
```
...
# Expected output:
# ✅ artist-api-dev (port 3000)
# ✅ cloudflare-tunnel (HTTPS)
# ✅ flutter-web (port 9000)
```

### Setup PM2 Auto-Start (One-time setup)
To ensure PM2 starts on boot:
```bash
# Save current PM2 processes
pm2 save

# Generate startup script (macOS)
pm2 startup
